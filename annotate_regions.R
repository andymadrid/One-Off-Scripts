...
build_annotation <- function(gtf = gencode_gtf(),
  ...
  gr <- keepSeqlevels(gr, intersect(keep_seqlevels, seqlevels(gr)),
                      pruning.mode = "coarse")
  ...
  tx    <- gr[ty == "transcript"]
  tss   <- keep3(resize(tx, 1L, fix = "start"))   # strand-aware
  ...
  exl <- reduce(split(granges(exons), exons$gene_id))
  gbl <- split(granges(genes), genes$gene_id)
  ids <- intersect(names(gbl), names(exl))
  il  <- psetdiff(unlist(range(gbl[ids]), use.names = FALSE), exl[ids])
...
annotate_regions <- function(regions, anno,
  ...
  prom <- promoters(f$tss, upstream = max(0L, as.integer(-tss_region[1])),
                    downstream = max(0L, as.integer(tss_region[2])) + 1L)
  dstr <- flank(f$genes, width = downstream, start = FALSE)
  ...
  if (flank_kb > 0) {
    hf <- findOverlaps(suppressWarnings(trim(gr + as.integer(flank_kb * 1000))),
